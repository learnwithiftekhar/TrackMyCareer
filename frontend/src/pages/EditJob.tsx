...
export default function EditJob() {
  ...
  useEffect(() => {
    if (!id) return;
    Promise.all([getJob(Number(id)), getCompanies()])
      .then(([job, comps]) => {
        setCompanies(comps);
        ...
        setForm({
          ...
          companySearch: job.companyName,
          jobUrl: job.jobUrl ?? '',
          status: job.appliedStatus as Status,
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      navigate(`/jobs/${id}`, { state: { saved: true } });
    } catch {
  ...
  if (loadError) {
    return (
      <main className="mx-auto max-w-[880px] px-8 py-7">
        <div className="text-[14px] text-[#94352a]">{loadError}</div>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-[880px] px-8 py-7 pb-24">
      ...
      <nav className="mb-[22px] flex items-center gap-2 text-[13px] text-muted-foreground">
        ...
        <Link to={`/jobs/${id}`} className="text-muted-foreground no-underline transition-colors hover:text-secondary-foreground">
          {form.jobTitle || 'Job'}
        </Link>
        <svg className="size-3" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <path d="M6 4l4 4-4 4" />
      ...
      <section className="mb-7">
        ...
        <p className="text-[14px] text-muted-foreground">Update the details for this job application.</p>
      </section>
      ...
      <div className="sticky bottom-4 mt-8 flex items-center justify-between gap-3 rounded-[12px] border border-[#ddd7c7] bg-card px-[18px] py-3 shadow-[0_4px_16px_-8px_rgba(31,29,26,.16),0_1px_2px_rgba(31,29,26,.04)]">
        <div className="flex items-center gap-2 text-[12.5px] text-muted-foreground">
          {submitError ? (
            ...
            <>
              <span className="size-1.5 rounded-full bg-[#b3afa3]" />
              Update the fields above and save
            </>
          )}
        </div>
